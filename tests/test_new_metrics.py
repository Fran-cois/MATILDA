        ...
        try:
            ...
            if isinstance(first_rule, dict):
                ...
                if has_orig_acc and has_orig_cov:
                    print(
                        f"  ✓ {run_dir.name}: Has original_accuracy={first_rule['original_accuracy']:.2f}, "
                        f"original_coverage={first_rule['original_coverage']:.2f}"
                    )
                    success_count += 1
                else:
                    print(
                        f"  ❌ {run_dir.name}: Missing fields (has_orig_acc={has_orig_acc}, has_orig_cov={has_orig_cov})"
                    )
                    fail_count += 1
            else:
...
def test_comparison_report():
    ...
    if has_quality_section:
        print("✓ Comparison report contains quality metrics section")
        return True
    else:
        print("❌ Comparison report missing quality metrics section")
        return False
