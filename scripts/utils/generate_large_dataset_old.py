...
for scalability testing. Supports multiple generation strategies.
"""
...
class LargeDatasetGenerator:
    ...
    def generate_synthetic_schema(self, num_tables: int = 5, 
        ...
        """
        Generate synthetic schema (tables and columns).
        
        :param num_tables: Number of tables to create.
        :param cols_per_table: Number of columns per table.
        :param with_relationships: Add foreign key constraints in schema.
        :return: List of (table_name, [column_names]) tuples.
        """
        schema = []
        for i in range(num_tables):
            ...
            # Add regular columns
            num_regular_cols = cols_per_table - 1
            if with_relationships and i > 0:
                # Reserve columns for foreign keys (1-2 refs to previous tables)
                num_refs = min(2, i)
                num_regular_cols = cols_per_table - 1 - num_refs
        ...
        # Parse column types (skip id column)
        col_defs = []
        for col_def in column_types[1:]:  # Skip id column
            parts = col_def.split()
            col_name = parts[0]
            col_type = parts[1].upper()
            is_fk = "REFERENCES" in col_def
            col_defs.append((col_name, col_type, is_fk))
        ...
        for batch_start in range(0, num_rows, batch_size):
            ...
            for row_id in range(batch_start + 1, batch_end + 1):
                ...
                fk_idx = 0
                for col_name, col_type, is_fk in col_defs:
                    if is_fk and foreign_keys and fk_idx < len(foreign_keys):
                        # Foreign key reference
                        ref_table, max_id = foreign_keys[fk_idx]
                        values.append(random.randint(1, min(max_id, row_id)))  # Valid FK
                        fk_idx += 1
                        ...
                        values.append(random.randint(1, 1000))  # Default
            batch_end = min(batch_start + batch_size, num_rows)
            batch_data = []
            ...
            for row_id in range(batch_start + 1, batch_end + 1):
                ...
                for col_idx in range(num_cols):
                    ...
                    val_type = random.choice(["int", "text", "float"])
                    if val_type == "int":
                        if foreign_keys and col_idx < len(foreign_keys):
                            # Foreign key reference
                            ref_table, max_id = foreign_keys[col_idx]
    ...
    def generate_dataset(self, target_tuples: int = 1000000,
        ...
        # Generate schema
        print("Step 1: Generating schema...")
        schema = self.generate_synthetic_schema(num_tables, cols_per_table)
        ...
        for idx, (table_name, columns) in enumerate(schema):
            # Bucolumn_types=columns,
            foreign_keys = None
            if with_relationships and idx > 0:
                ...
                foreign_keys = []
                for ref_idx in range(num_refs):
                    ref_table = schema[ref_idx][0]
                    ref_max_id = table_ids[ref_table]
                    foreign_keys.append((ref_table, ref_max_id))
            
            self.populate_table(
                table_name=table_name,
                num_cols=cols_per_table - 1,  # Exclude id
                num_rows=rows_per_table,
                foreign_keys=foreign_keys
            )
            
            table_ids[table_name] = rows_per_table
...
Examples:
  ...
  python scripts/utils/generate_large_dataset.py data/large_scale/dataset_1M.db --stats-only
    ...
    parser.add_argument('--tuples', '-t', type=int, default=1000000,
                       help='Target number of tuples (default: 1,000,000)')
    parser.add_argument('--tables', '-T', type=int, default=5,
    ...
    generator = LargeDatasetGenerator(args.output, seed=args.seed)
    ...
    if args.stats_only:
        ...
        generator.generate_dataset(
            target_tuples=args.tuples,
            num_tables=args.tables,
            cols_per_table=args.columns,
