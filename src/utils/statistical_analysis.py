...
def compute_statistics(values: List[float], metric_name: str = "metric") -> PerformanceStats:
    ...
    if len(arr) > 1:
        ci = stats.t.interval(confidence=0.95, df=len(arr) - 1, loc=mean_val, scale=stats.sem(arr))
...
def analyze_rules_performance(
    rules_file: Path, metrics: Optional[List[str]] = None
...
def analyze_time_metrics(
    time_metrics_file: Path, metrics: Optional[List[str]] = None
) -> Dict[str, PerformanceStats]:
    """
        ...
        try:
            if use_parametric and len(values1) >= 2 and len(values2) >= 2:
                test_result = perform_t_test(
                    values1, values2, metric, algorithm1_name, algorithm2_name
                )
            else:
...
def generate_statistical_report(
    ...
    for algo in algorithms:
        ...
        if include_time_metrics:
            time_pattern = "init_time_metrics_*.json"
            for file_path in results_dir.glob(time_pattern):
                dataset_name = file_path.stem.replace("init_time_metrics_", "")
                if (
                    dataset_name in result_files[algo]
                ):  # Only include if we have results for this dataset
            ...
            for dataset in common_datasets:
                try:
                    comparisons = compare_algorithms(
                        result_files[algo1][dataset], result_files[algo2][dataset], algo1, algo2
                    )
                ...
                if (
                    ...
                    and dataset in time_files.get(algo1, {})
                    and dataset in time_files.get(algo2, {})
                ):
                    try:
                        time_comparisons = compare_time_metrics(
                            time_files[algo1][dataset], time_files[algo2][dataset], algo1, algo2
                        )
                        ...
                        print(
                            f"Warning: Could not compare time metrics for {algo1} vs {algo2} on {dataset}: {e}"
                        )

    # Generate summary
    report["summary"]["total_algorithms"] = len(algo_list)
    report["summary"]["total_datasets"] = len(
        set(dataset for datasets_dict in result_files.values() for dataset in datasets_dict.keys())
    )
...
def format_significance_test_markdown(test: SignificanceTest) -> str:
    ...
    return (
        ...
        f"{sig_indicator} | {f'{test.effect_size:.4f}' if test.effect_size is not None else 'N/A'} |"
    )
