...
def init(
    ...
    :param max_nb_occurrence_per_table_and_column: Dictionary specifying max occurrences per table and column
    :param results_path: Path to store results if needed
    :param compatibility_mode: Mode de compatibilité à utiliser (voir CompatibilityChecker.MODE_*)
...
def discover_tgd_rules(
    ...
    results_path: str = None,
    compatibility_mode: str = None
) -> Generator[TGDRule, None, None]:
    """
    Découvre les règles TGD en utilisant le mode de compatibilité spécifié.
    ...
    :param results_path: Path to store results if needed
    :param compatibility_mode: Mode de compatibilité à utiliser
    :return: Generator yielding discovered TGD rules
    """
    # Initialiser le graphe de contraintes avec le mode de compatibilité spécifié
    cg, mapper, jia_list = init(
        db_inspector,
        max_nb_occurrence=max_nb_occurrence,
        results_path=results_path,
        compatibility_mode=compatibility_mode
    )
    
    if not cg or not mapper:
        logging.error("Failed to initialize constraint graph or mapper")
        return
    
    # Utiliser le DFS pour découvrir les règles candidates
    for candidate_rule in dfs(
        graph=cg,
        start_node=None,
        pruning_prediction=path_pruning,
        db_inspector=db_inspector,
        mapper=mapper,
        max_table=max_table,
        max_vars=max_vars
    ):
        # Pour chaque règle candidate, trouver les différentes façons de la diviser
        splits = split_candidate_rule(candidate_rule)
        
        for split in splits:
            body, head = split
            
            # Vérifier si la règle satisfait nos critères de qualité
            should_keep, support, confidence = split_pruning(
                candidate_rule, body, head, db_inspector, mapper
            )
            
            if should_keep:
                # Générer la représentation textuelle de la TGD
                tgd_string = instantiate_tgd(candidate_rule, split, mapper)
                
                # Convertir en objet TGDRule
                if tgd_string:
                    try:
                        tgd_rule = str_to_tgd(tgd_string, support, confidence)
                        yield tgd_rule
                    except ValueError as e:
                        logging.error(f"Error creating TGD rule: {e}")
                        
def instantiate_tgd_object(candidate_rule, split, mapper, support=1.0, confidence=1.0):
    """
    Crée directement un objet TGDRule à partir d'un candidat et d'un split.
    
    :param candidate_rule: Le candidat de règle (liste de JoinableIndexedAttributes)
    :param split: La division du candidat en corps/tête (body, head)
    :param mapper: Le mappeur d'attributs
    :param support: Le support de la règle
    :param confidence: La confiance de la règle
    :return: Un objet TGDRule ou None si l'instanciation échoue
    """
    try:
        ...
        import logging
        logging.getLogger(__name__).exception(f"Error instantiating TGD object: {e}")
        return None
